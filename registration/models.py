...
class Title(models.Model):
    ...
    on_ek = models.BooleanField(u'Ön ek olarak kullan', help_text=u'Eğer işaretlerseniz kullanıcıların isminin önüne unvan eklenir.')
    aciklama = Translate
...
class User(AbstractUser):
    ...
    university = models.CharField(_('University'), max_length=50, blank=True, null=True)
    department = models.CharField(_('Department'), max_length=50, blank=True, null=True)
    address = models.TextField(_('Address'), blank=True, null=True)
    country = models.CharField(_('Country'),max_length=2, default='TR', choices=COUNTRIES, blank=True, null=True)
    phone = models.CharField(_('Phone'),max_length=20, blank=True, null=True)
    
    @property
    def get_full_name_tr(self):
        """
        Returns the first_name plus the last_name, with a space in between.
        """
        if self.title and self.title.on_ek:
            full_name = '%s %s %s' % (self.title.aciklama_tr, self.first_name, self.last_name)
        else:
    ...
    def get_full_name_en(self):
        ...
        """
        if self.title and self.title.on_ek:
            full_name = '%s %s %s' % (self.title.aciklama_en, self.first_name, self.last_name)
        else:
            full_name = '%s %s' % (self.first_name, self.last_name)
        return full_name.strip()
    
    get_full_name = Translate
...
class AccomodationOption(models.Model):
    __metaclass__ = LocalizeModelBase
    ...
    class Meta:
        verbose_name = _('Accomodation Option')
        verbose_name_plural = _('Accomodation Options')
        
class Accomodation(models.Model):
    user = models.ForeignKey(User, unique=True)
    option = models.ForeignKey(AccomodationOption, verbose_name=_(u'Accomodation Option'), blank=True, null=True)
...
class RegistrationManager(models.Manager):
    ...
    def create_inactive_user(self, username, email, password,
                             site=None, send_email=True, request=None, **kwargs):
        """
        ...
        """
        new_user = UserModel().objects.create_user(username, email, password, **kwargs)
